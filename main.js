/* ============================================================
   ...
   ADDITIONS (FULL "FIRST 15" MVP):
   - Jump + gravity + ground collision
   - First/third person toggle (V) + head bob
   - Pause menu (ESC) + settings (sens/FOV/vol/fog)
   - FPS counter
   - Dialogue system (F near NPC)
   ============================================================ */
...
(function injectCSS(){
  const css = `
    ...
    #overlay .sub{
      opacity:.85; font:800 14px system-ui, -apple-system, Segoe UI, Roboto, Arial;
    ...
    .hudRight{
      left:auto; right:14px; min-width: 210px; text-align:left;
    }
    ...
    /* Pause / Settings / Dialogue panels */
    .overlayWrap{
      position:fixed; inset:0; display:none; align-items:center; justify-content:center;
    ...
    .settingRow{ display:flex; gap:12px; align-items:center; flex-wrap:wrap; margin-top:12px; }
    .slider{ width:280px; }
    .kbd{ padding:3px 8px; border-radius:10px; background:rgba(255,255,255,.08); border:1px solid rgba(255,255,255,.12); font-weight:900; }
  `;
...
overlay.innerHTML = `
  <div>
    Click to enter
    <div class="sub">
      <span class="kbd">WASD</span> move • <span class="kbd">Shift</span> sprint • <span class="kbd">Space</span> jump •
      <span class="kbd">Mouse</span> look • <span class="kbd">E</span> play • <span class="kbd">F</span> talk •
      <span class="kbd">V</span> camera • <span class="kbd">Esc</span> pause/close
    </div>
  </div>
`;
...
hud.className = "hud";
hud.innerHTML = `
  <div><b>WASD</b> move • <b>Shift</b> sprint • <b>Space</b> jump • <b>Mouse</b> look • <b>V</b> camera</div>
  <div><b>E</b> interact (games) • <b>F</b> talk (NPC) • <b>Esc</b> pause / close UI</div>
`;
...
hudRight.innerHTML = `
  <div style="display:flex; justify-content:space-between; gap:10px;">
    <div><b>BALANCE</b></div>
    <div id="hudFps" style="opacity:.9;">FPS: --</div>
  </div>
  <div id="hudBal">$1000</div>
  <div id="hudCam" style="opacity:.9; margin-top:6px;">CAM: 1P</div>
`;
...
/* ------------------- Pause / Settings / Dialogue UI -------- */
const pauseWrap = document.createElement("div");
pauseWrap.className = "overlayWrap";
pauseWrap.innerHTML = `
  <div class="uiCard">
    <div class="uiHead">
      <div class="uiTitle">PAUSED</div>
      <button class="uiClose" id="p_close">Close</button>
    </div>
    <div class="uiBody">
      <div class="uiBtns">
        <button class="btn btnPrimary" id="p_resume">Resume</button>
        <button class="btn" id="p_settings">Settings</button>
        <button class="btn" id="p_unlock">Unlock Mouse</button>
      </div>
      <div class="msg">Tip: <span class="kbd">Esc</span> toggles pause • <span class="kbd">V</span> camera • <span class="kbd">F</span> talk</div>
    </div>
  </div>
`;
...
settingsWrap.className = "overlayWrap";
settingsWrap.innerHTML = `
  <div class="uiCard">
    <div class="uiHead">
      <div class="uiTitle">SETTINGS</div>
      <button class="uiClose" id="s_close">Back</button>
    </div>
    <div class="uiBody">
      <div class="settingRow">
        <div class="pill" style="min-width:220px;"><b>Mouse Sens</b> <span id="sensVal"></span></div>
        <input class="slider" id="sens" type="range" min="0.0015" max="0.009" step="0.0001" />
      </div>
      <div class="settingRow">
        <div class="pill" style="min-width:220px;"><b>FOV</b> <span id="fovVal"></span></div>
        <input class="slider" id="fov" type="range" min="60" max="110" step="1" />
      </div>
      <div class="settingRow">
        <div class="pill" style="min-width:220px;"><b>Master Volume</b> <span id="volVal"></span></div>
        <input class="slider" id="vol" type="range" min="0" max="1" step="0.01" />
      </div>
      <div class="settingRow">
        <label class="pill" style="min-width:220px; cursor:pointer;">
          <input id="fogToggle" type="checkbox" />
          <b>Fog</b>
        </label>
      </div>
      <div class="msg">These save automatically.</div>
    </div>
  </div>
`;
...
dialogueWrap.className = "overlayWrap";
dialogueWrap.innerHTML = `
  <div class="uiCard">
    <div class="uiHead">
      <div class="uiTitle" id="dlgName">NPC</div>
      <button class="uiClose" id="d_close">Close</button>
    </div>
    <div class="uiBody">
      <div class="msg" id="dlgText"></div>
      <div class="uiBtns" id="dlgBtns"></div>
    </div>
  </div>
`;
document.body.appendChild(dialogueWrap);
...
function updateHUD(){
  hudBal.textContent = `$${Economy.balance}`;
}
updateHUD();
...
/* ---------------------- Settings --------------------------- */
const SETTINGS_KEY = "casino_settings_v2";
const Settings = {
  mouseSens: 0.0038,
  fovBase: 70,
  volume: 0.70,
  ...
  load(){
    try{
      ...
      if (typeof s.mouseSens === "number") this.mouseSens = s.mouseSens;
      if (typeof s.fovBase === "number") this.fovBase = s.fovBase;
      if (typeof s.volume === "number") this.volume = s.volume;
      if (typeof s.fog === "boolean") this.fog = s.fog;
    } catch {}
  },
  save(){
  ...
  apply(){
    ...
    camera.fov = this.fovBase;
    camera.updateProjectionMatrix();
    scene.fog = this.fog ? new THREE.FogExp2(0x06050a, 0.018) : null;
  }
};
Settings.load();
let MOUSE_SENS = Settings.mouseSens;
...
/* ----------------- World: Bellagio-ish --------------------- */
const WORLD = { W: 60, D: 50, H: 9 };

const wallTex = canvasTex((g,s)=>{
  g.fillStyle="#f3efe7"; g.fillRect(0,0,s,s);
...
const carpetTex = canvasTex((g,s)=>{
  g.fillStyle="#5b0f1c"; g.fillRect(0,0,s,s);
...
const marbleTex = canvasTex((g,s)=>{
  g.fillStyle="#14141a"; g.fillRect(0,0,s,s);
  for (let i=0;i<260;i++){
...
/* Room shell */
const room = new THREE.Mesh(new THREE.BoxGeometry(WORLD.W, WORLD.H, WORLD.D), ivoryWallMat);
room.position.y = WORLD.H/2;
room.receiveShadow = true;
scene.add(room);

/* Floor zones */
const marbleAisle = new THREE.Mesh(new THREE.PlaneGeometry(10, WORLD.D-6), marbleMat);
...
function addPaintingOnWall(side, x, y, z){
...
addPaintingOnWall("left",  -WORLD.W/2 + 0.6, 5.0, -10);
addPaintingOnWall("right",  WORLD.W/2 - 0.6, 5.0,  10);
addPaintingOnWall("back",   10, 5.0, -WORLD.D/2 + 0.6);
addPaintingOnWall("back",  -10, 5.0, -WORLD.D/2 + 0.6);
...
/* Apply settings now that scene/camera exist */
Settings.apply();
...
const keys = { w:false,a:false,s:false,d:false, shift:false, space:false, e:false, f:false, v:false };
document.addEventListener("keydown", (ev)=>{
  ...
  if (ev.code==="KeyF") keys.f=true;
  if (ev.code==="KeyV") keys.v=true;

  if (ev.code==="Escape") {
    // priority: close dialogue/settings/pause, else close game UI, else open pause
    if (dialogueWrap.style.display === "flex") { dialogueClose(); return; }
    if (settingsWrap.style.display === "flex") { settingsClose(); return; }
    if (pauseWrap.style.display === "flex") { pauseClose(); return; }
...
document.addEventListener("keyup", (ev)=>{
  ...
  if (ev.code==="KeyF") keys.f=false;
  if (ev.code==="KeyV") keys.v=false;
});
...
function canLookMove(){
  return started && locked && !UI.anyOpen() && !CIN.active && pauseWrap.style.display !== "flex" && settingsWrap.style.display !== "flex" && dialogueWrap.style.display !== "flex";
}
...
const player = {
  ...
  camMode: 0 // 0 = first person, 1 = third person
};
camera.position.copy(player.pos);

function updateMovement(dt){
  if (!started || UI.anyOpen() || CIN.active || pauseWrap.style.display==="flex" || settingsWrap.style.display==="flex" || dialogueWrap.style.display==="flex") return;

  // V toggles camera mode
  if (keys.v){
    keys.v = false;
    player.camMode = player.camMode ? 0 : 1;
    hudCam.textContent = `CAM: ${player.camMode ? "3P" : "1P"}`;
  }
  ...
  // Sprint FOV blend around base FOV
  const moving = player.vel.lengthSq() > 0.25;
  const desiredFov = (keys.shift && moving) ? (Settings.fovBase + 4) : Settings.fovBase;
  camera.fov = approach(camera.fov, desiredFov, 12*dt);
  camera.updateProjectionMatrix();

  // Head bob (first person only)
  if (moving && player.grounded){
    player.bobT += dt * (keys.shift ? 12 : 9);
  } else {
    player.bobT = approach(player.bobT, 0, dt * 6);
  }
  const bob = (player.camMode===0 && moving) ? Math.sin(player.bobT) * 0.045 : 0;

  if (player.camMode === 0){
    camera.position.set(player.pos.x, player.pos.y + bob, player.pos.z);
    camera.rotation.set(pitch, yaw, 0);
  } else {
    // Third-person chase camera behind the player
    const back = new THREE.Vector3(0, 0, 1).applyAxisAngle(new THREE.Vector3(0,1,0), yaw);
    const camPos = player.pos.clone()
      .add(new THREE.Vector3(0, 0.25, 0))
      .addScaledVector(back, 3.3);

    // keep camera within bounds a bit
    camPos.x = THREE.MathUtils.clamp(camPos.x, -bx, bx);
    camPos.z = THREE.MathUtils.clamp(camPos.z, -bz, bz);
    camPos.y = Math.max(camPos.y, 1.0);

    camera.position.lerp(camPos, 1 - Math.pow(0.001, dt)); // smooth
    const look = player.pos.clone().add(new THREE.Vector3(0, 0.9, 0));
    camera.lookAt(look);
  }
}
...
/* ------------------- Pause / Settings / Dialogue logic ----- */
function pauseOpen(){
  if (UI.anyOpen() || CIN.active) return;
  pauseWrap.style.display = "flex";
}
function pauseClose(){ pauseWrap.style.display = "none"; }

function settingsSyncUI(){
  const sens = document.getElementById("sens");
  const fov = document.getElementById("fov");
  const vol = document.getElementById("vol");
  const fog = document.getElementById("fogToggle");
  sens.value = String(Settings.mouseSens);
  fov.value = String(Settings.fovBase);
  vol.value = String(Settings.volume);
  fog.checked = Settings.fog;
  document.getElementById("sensVal").textContent = Settings.mouseSens.toFixed(4);
  document.getElementById("fovVal").textContent = Settings.fovBase.toFixed(0);
  document.getElementById("volVal").textContent = Math.round(Settings.volume*100) + "%";
}
...
function dialogueClose(){ dialogueWrap.style.display = "none"; }

document.getElementById("p_close").onclick = pauseClose;
document.getElementById("p_resume").onclick = pauseClose;
document.getElementById("p_settings").onclick = settingsOpen;
document.getElementById("p_unlock").onclick = ()=> document.exitPointerLock?.();
document.getElementById("s_close").onclick = settingsClose;
document.getElementById("d_close").onclick = dialogueClose;
...
document.getElementById("fov").addEventListener("input", (e)=>{
  Settings.fovBase = parseFloat(e.target.value);
  document.getElementById("fovVal").textContent = Settings.fovBase.toFixed(0);
  Settings.apply();
  Settings.save();
});
...
const CIN = {
  ...
  start({toPos, toLook, durIn=0.75, durOut=0.75, onArrive, onReturn}){
    ...
    camera.fov = Math.max(60, Settings.fovBase - 4);
    camera.updateProjectionMatrix();
...
function makePerson(){
  ...
  g.userData = {
    ...
    name: (["Dealer","High Roller","Tourist","Pit Boss","VIP Guest","Local"][(Math.random()*6)|0]),
    mood: (["chill","lucky","bored","hyped","skeptical"][(Math.random()*5)|0]),
  };
...
function updateNPCs(dt){
  ...
  for (const n of NPC.list){
    ...
    // freeze while cinematic/UI/pause/dialogue open
    if (CIN.active || UI.anyOpen() || pauseWrap.style.display==="flex" || settingsWrap.style.display==="flex" || dialogueWrap.style.display==="flex") continue;
...
function buildRoulette(){
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.82, 0.86, 0.95, 32), woodMat);
...
function buildSlots(){
  ...
  const bank = new THREE.Mesh(new THREE.BoxGeometry(3.2, 1.0, 1.2), woodMat);
...
function buildBlackjack(){
  ...
  const base = new THREE.Mesh(new THREE.BoxGeometry(2.25, 0.78, 1.15), woodMat);
...
function handleInteract(){
  ...
  if (pauseWrap.style.display==="flex" || settingsWrap.style.display==="flex" || dialogueWrap.style.display==="flex") return;
...
async function runCinematicAction({ camPresetFn, actionFn, onDoneMsgFn }){
...
async function rouletteSpinAction(){
  ...
  await new Promise((res)=>{ Roulette._resolve = res; });
...
document.getElementById("r_spin").onclick = async ()=>{
  if (Roulette.actionInProgress) return;
  await runCinematicAction({ camPresetFn: CAM.rouletteAction, actionFn: rouletteSpinAction, onDoneMsgFn: ()=> rouletteRefresh() });
};
document.getElementById("s_spin").onclick = async ()=>{
  if (Slots.spinning) return;
  await runCinematicAction({ camPresetFn: CAM.slotsAction, actionFn: slotsSpinAction, onDoneMsgFn: ()=> slotsRefresh() });
};
document.getElementById("b_deal").onclick = async ()=>{
  await runCinematicAction({ camPresetFn: CAM.bjAction, actionFn: bjDealAction, onDoneMsgFn: ()=> bjRefresh() });
};
...
function updateHoverPrompt(){
  ...
  if (pauseWrap.style.display==="flex" || settingsWrap.style.display==="flex" || dialogueWrap.style.display==="flex"){
    showPrompt(false);
    return;
  }

  // If near NPC, prefer talk prompt
  const npc = nearestNPC(2.2);
  if (npc){
    showPrompt(true, "Press F to talk");
    return;
  }
...
/* ---------------------- Dialogue System -------------------- */
function nearestNPC(maxDist=2.2){
  if (!started) return null;
  const p = new THREE.Vector3(player.pos.x, 0, player.pos.z);
  let best=null, bestD=maxDist;
  for (const n of NPC.list){
    const d = p.distanceTo(new THREE.Vector3(n.position.x,0,n.position.z));
    if (d < bestD){
      bestD = d;
      best = n;
    }
...
function dialogueOpenFor(npc){
  ...
  dlgName.textContent = `${u.name}`;
  const opener = {
    chill: "Quiet night. You playing roulette or just vibing?",
    lucky: "I’m feeling lucky. You should hit the tables.",
    bored: "Same old machines… unless you’re about to change that.",
    hyped: "This place is alive! Let’s see a big win.",
    skeptical: "House always has an edge… but you can still run hot."
  }[u.mood] || "Enjoy the casino.";

  dlgText.textContent = opener;
  dlgBtns.innerHTML = "";
  ...
  mk("Any tips?", ()=>{
    dlgText.textContent = "Blackjack: don’t chase. Roulette: bet small. Slots: it’s pure vibes. Manage your bankroll.";
  });
  mk("Where’s the best game?", ()=>{
    dlgText.textContent = "If you want control: blackjack. If you want adrenaline: roulette. If you want chaos: slots.";
  });
  mk("Give me a lucky charm (+$25)", ()=>{
    Economy.pay(25);
    playWin();
    updateHUD();
    dlgText.textContent = "Alright… don’t tell the pit boss. (+$25)";
  });
  mk("Close", ()=> dialogueClose());
...
document.addEventListener("keydown", (ev)=>{
  if (ev.code === "KeyF"){
    if (!started) return;
    if (UI.anyOpen() || CIN.active) return;
    if (pauseWrap.style.display==="flex" || settingsWrap.style.display==="flex") return;

    const npc = nearestNPC(2.2);
    if (npc) dialogueOpenFor(npc);
  }
});
...
function updateFPS(dt){
  ...
  if (FPS.t >= 0.5){
    ...
    FPS.frames = 0;
    FPS.t = 0;
    hudFps.textContent = `FPS: ${FPS.value}`;
  }
}
